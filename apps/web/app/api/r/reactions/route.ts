...
// GET /api/reactions?slug=…  → list of reactions for a share.
//
// No auth gate on reads — anyone who can see the share page can see
// who reacted. Slug validation guards against SQL surprises.
...
//
// As of the activity-sidebar build, reactions are authored: the
// better-auth session cookie (cross-subdomain on .captureflow.xyz)
// must resolve to a real user, otherwise we 401. The DB stores both
// userId + display name so the sidebar can render attribution without
// a join.
export async function POST(req: NextRequest) {
  ...
  // Decorate with the visitor's avatar — see same note in /api/comments.
