...
//
// In production, share/snap posters + videos are read straight from the
// CDN origin (cdn.captureflow.xyz). But miniflare's local R2 (.wrangler/)
// has no HTTP origin, so a share recorded against `next dev` can't be
// played in the editor or viewer — the <video>/<img> point at a CDN that
// doesn't have the local bytes and spin forever.
//
// Pointing the local R2 base at this route lets those elements stream the
// bytes same-origin from the BUCKET binding, with Range support so the
// player can seek. The storage key rides in the catch-all PATH
// (…/api/r/media/videos/<slug>.mp4) rather than a query param, so the
// editor's `?v=<sizeBytes>` cache-buster stays a clean, separate query.
// Wire it via:
...
//
// Guarded to localhost: in production this route 404s, so it can never be
// used to stream bytes around the private-share visibility gate that
// /api/r/download enforces.
