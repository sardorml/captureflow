 ...
 */
export type RecordingPlayerHandle = {
  ...
  // Seek without changing play state — the edit-page timeline scrubs the
  // playhead without yanking the video into playback on every drag frame.
...
type Props = {
  ...
  webcamUrl?: string;
  // Authoritative duration from the recording record, used to size the scrubber before <video> knows its own duration — which can take seconds, or never resolve to a finite number for fragmented MP4s.
 ...
 */
export const RecordingPlayer = forwardRef<RecordingPlayerHandle, Props>(
  function RecordingPlayer(
    ...
    /*
     * Seed mute from the persisted config so a deliberately-silent recording
    ...
    /*
     * Fragmented MP4s (and some recording renders, depending on the muxer
    ...
    const seekTo = (fraction: number): void => {
       ...
       * dragging feel laggy and screenshot to keyframes.
       */
      setCurrentTime(next);
    };
          ...
          style={
            isFullscreen
              ...
              : {
                  ...
                  ...recordingBackgroundStyle(config.background),
                }
          }
        ...
        >
          ...
          >
            <video
              ...
              onTimeUpdate={(e) => {
                 ...
                 * — browsers screenshot seeks to keyframes, yanking the bar away
          ...
          {overlayIcon && isPlaying && !ended ? (
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
              <div
                key={overlayIcon}
                className="animate-recording-icon-pop flex h-32 w-32 items-center justify-center rounded-full bg-[#171717]/90"
              >
                {overlayIcon === "pause" ? (
...
function webcamCornerClass(corner: RecordingCameraCorner): string {
  switch (corner) {
    case "bottom-right":
...
// studio bubble.
function webcamSizeClass(size: RecordingCameraSize): string {
  switch (size) {
    case "small":
...
function recordingBackgroundStyle(background: string): React.CSSProperties {
  if (background === "transparent") return {};
  if (isRecordingHexColor(background)) return { backgroundColor: background };
  if (isRecordingGradientKey(background)) {
    return {
      background: recordingGradientCss(
        RECORDING_GRADIENT_PRESETS[background].stops,
      ),
    };
  }
  return {};
