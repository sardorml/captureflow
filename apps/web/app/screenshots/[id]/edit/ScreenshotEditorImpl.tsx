...
export function ScreenshotEditorImpl(props: ScreenshotEditorProps) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        img.onerror = () => {
          if (!cancelled)
            setImageLoadError("Failed to decode screenshot image.");
        };
        img.src = objectUrl;
      } catch {
        if (!cancelled) {
          setImageLoadError(
            "Could not load the screenshot image. The bucket may have lost CORS access.",
          );
        }
      }
    })();
  ...
  const commitTitle = async (): Promise<void> => {
    ...
    setTitleError(null);
    try {
      const res = await renameScreenshotAction(screenshotId, next);
      if (res.error) {
  ...
  const handleSave = async (): Promise<void> => {
    ...
    try {
      ...
      const res = await saveScreenshotAction(screenshotId, blob, {
      ...
      <header className="relative flex h-20 shrink-0 items-center justify-between border-b border-line px-4">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2 lg:max-w-[calc(50%-220px)]">
          <IconButton
            ...
            tooltip="Back to screenshot"
            ariaLabel="Back to screenshot"
          >
          ...
          {titleEditing ? (
            ...
            >
              <span className="truncate text-sm text-neutral-200">
                {title?.trim() || "Untitled screenshot"}
              </span>
