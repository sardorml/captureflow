...
// Public share viewer links are built by the canonical helper so the
// merged `/r/<id>` path + host stay in one place (lib/site.ts). The CDN
// origin for poster/video reads honors NEXT_PUBLIC_R2_PUBLIC_BASE_URL so
// local dev can point card thumbnails at the local R2 proxy; prod falls
// back to cdn.captureflow.xyz.
const CDN_BASE_URL =
...
type SharesListProps = {
  ...
  // Workspace policy: when false, the Public option is hidden from
  // the visibility select for new flips. Defaults to true to preserve
  // pre-policy behaviour.
  allowPublicLinks?: boolean;
  // user_id → display name for every workspace member, so we can
  // surface the author chip on every card (owner included).
  ownerNames?: Record<string, string>;
  // user_id → avatar URL — same lookup, rendered as <AvatarImage> on
  // each card when present, otherwise the seeded fallback.
...
function ShareCard({
  ...
  // When the overflow menu is open we lock the hover cluster to
  // visible so the menu doesn't appear to float over a hidden anchor
  // (mouse leaves the thumbnail while the popover sticks open).
  ...
  const posterUrl = share.posterKey
    ...
    : null;
  // Streaming-upload shares don't generate a static poster; pull the
  // first frame straight from the source video with preload=metadata
  // so the browser fetches ~100 KB instead of the whole file.
  const videoThumbUrl =
    share.state === 'ready' && !posterUrl
      ? `${CDN_BASE_URL}/${share.storageKey}?v=${share.sizeBytes}`
      : null;
  // Card click always opens the public share page (in a new tab —
  // cross-subdomain, and the dashboard tab stays put). The pencil/
  // settings icon in the row actions is the path to the editor.
  ...
  return (
    <article className="group overflow-hidden rounded-lg border border-line bg-neutral-900 transition-colors hover:border-line-strong">
      ...
      >
        ...
        {/* Hover-revealed action cluster — Loom posture. Pinned to the
            top-right of the thumbnail; eats clicks (stopPropagation +
            preventDefault) so the underlying Link doesn't fire. */}
        <div
          className={
      ...
      <div className="space-y-3 p-4">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="flex min-w-0 flex-col leading-tight">
            ...
            {/* Visibility as plain text + chevron, Loom posture. Same
                modal trigger as before; the chip styling is gone so it
                reads as metadata, not chrome. */}
            {canManage ? (
              <VisibilityDialog
                value={visibility}
                disabled={pending}
...
// Plain-text visibility affordance, Loom posture — sits directly
// under the author name with a small chevron to hint "click to change."
// No background, no border; reads as metadata, not as a control. When
// non-interactive (viewer is read-only) we render a <span> so it
// doesn't look pressable.
