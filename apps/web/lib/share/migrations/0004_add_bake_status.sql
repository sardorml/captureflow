-- Tracks whether a share is still being post-processed by the desktop
...
-- finishes baking them in. Anyone who opens the share between
-- /api/finalize and /api/replace would otherwise see the bare screen
-- recording and assume the share is broken.
--
-- States:
--   'none'     — no bake is expected (no cam, no cursor, no bg).
--   'expected' — bake is expected; share page shows an "applying…" hint
--                and polls /api/state until this column flips.
--   'done'     — bake has landed; share page renders normally.
--
