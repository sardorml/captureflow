-- Adds the share title (rendered above the player) and a separate
-- table for reactions (anonymous emoji taps recorded at a video
-- timestamp).
--
-- Title is nullable: existing rows + future shares without a window
-- name fall through to the slug-only header. Default-emoji set lives
-- in app code (lib/share/reactions.ts), so the table holds raw
