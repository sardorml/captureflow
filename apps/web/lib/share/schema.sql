-- Share infrastructure schema for Cloudflare D1 (SQLite).
--
...
CREATE TABLE IF NOT EXISTS shares (
  ...
  title             TEXT,
  state             TEXT NOT NULL,
  -- Owning better-auth user (nullable for legacy / anonymous shares).
  ...
  -- via a follow-up /api/replace. 'expected' tells the share page to
...
-- Drives the user dashboard's "my shares" query.
CREATE INDEX IF NOT EXISTS idx_shares_user
  ON shares (user_id, created_at DESC)
  WHERE user_id IS NOT NULL;

-- Partial index drives the daily retention sweep — only ready shares
...
-- shares need to be aborted, and we look up by created_at.
...
-- single small POST; the share page groups adjacent timestamps into
...
-- versa). The column is named `active_shares_override` for back-
-- compat with the pre-snaps schema; its semantic now covers the
-- combined shares + snaps artifact count.
