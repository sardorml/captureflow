...
export type ShareRow = {
  ...
  userId: string | null;
  // Workspace owning the share, stamped at upload time so the viewer auth gate can check membership in one indexed lookup (no user_id → workspace_member hop). Null for legacy anonymous uploads.
...
export type InitRequest = {
  ...
  hasWebcam?: boolean;
  // Target workspace for the new share. The bearer user must be a member; on mismatch we silently fall back to their personal workspace so a stale client never blocks an upload. Omitted means personal workspace.
  workspaceId?: string;
};
