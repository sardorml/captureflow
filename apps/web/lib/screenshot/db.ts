...
function rowFromD1(r: D1ScreenshotRow): ScreenshotRow {
  return {
    id: r.id,
    userId: r.user_id,
...
export async function insertScreenshot(row: ScreenshotRow): Promise<void> {
  const d = await db();
  await d
    .prepare(
      `INSERT INTO screenshots (${COLUMNS})
...
export async function getScreenshot(id: string): Promise<ScreenshotRow | null> {
  const d = await db();
  const r = await d
    .prepare(`SELECT ${COLUMNS} FROM screenshots WHERE id = ?1 LIMIT 1`)
    .bind(id)
    .first<D1ScreenshotRow>();
  return r ? rowFromD1(r) : null;
}

// name + email are nullable: the LEFT JOIN can miss if the user was hard-deleted.
export type ScreenshotWithOwner = ScreenshotRow & {
  ownerName: string | null;
  ownerEmail: string | null;
};

export async function getScreenshotWithOwner(
  id: string,
): Promise<ScreenshotWithOwner | null> {
  const d = await db();
  type Row = D1ScreenshotRow & {
...
export async function updateScreenshotAfterEdit(
  ...
  const sql = `UPDATE screenshots SET ${sets.join(
    ", ",
...
export async function updateScreenshotVisibility(
  id: string,
  ...
  const r = await d
    .prepare(
      `UPDATE screenshots SET visibility = ?2, updated_at = ?3 WHERE id = ?1 RETURNING ${COLUMNS}`,
    )
...
export async function softDeleteScreenshot(id: string): Promise<boolean> {
  const d = await db();
  const res = await d
    .prepare(
      `UPDATE screenshots SET state = 'deleted', updated_at = ?2 WHERE id = ?1`,
    )
    .bind(id, Date.now())
    .run();
...
export async function bumpScreenshotLastViewed(id: string): Promise<void> {
  const d = await db();
  await d
    .prepare(
      `UPDATE screenshots
