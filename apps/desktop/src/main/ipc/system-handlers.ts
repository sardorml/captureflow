...
export function registerSystemHandlers(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.REQUEST_MIC_PERMISSION, () =>
    requestMicPermission(),
  );
  ...
  ipcMain.handle(
    ...
    async (event, kind: MediaPermissionKind): Promise<boolean> => {
      ...
      const allow = await openPermissionDialogWindow(win, {
        kind,
        variant: "denied",
      });
      if (allow) {
  ...
  ipcMain.handle(
    ...
    async (_event, payload: BugReportPayload): Promise<BugReportResult> => {
      ...
      if (!formikKey) {
        ...
        return {
          ok: false,
          error: "Bug reporting is not configured in this build",
        };
      }
      ...
      try {
        ...
        let parsed: { success?: string | boolean; message?: string } | null =
          null;
        try {
        ...
        const succeeded =
          res.ok && (parsed?.success === true || parsed?.success === "true");
