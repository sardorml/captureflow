...
} from "../lib/share/share-account-actions";
import {
  clearShareAuth,
  getShareAuthState,
  onShareAuthChange,
} from "../lib/share/share-auth";
import {
  getShareConnectivity,
  onShareConnectivityChange,
} from "../lib/share/share-connectivity";
import {
  getShareUsage,
  onShareUsageChange,
  refreshShareUsage,
} from "../lib/share/share-usage";
import {
  getWorkspacesState,
  onWorkspacesChange,
  refreshWorkspaces,
...
export function registerShareAuthHandlers(): void {
  ...
  ipcMain.handle(
    IPC_CHANNELS.SHARE_AUTH_SIGN_OUT,
    async (): Promise<ShareAuthState> => {
  ...
  ipcMain.handle(
    IPC_CHANNELS.SHARE_CONNECTIVITY_GET,
  ...
  ipcMain.handle(
    IPC_CHANNELS.SHARE_USAGE_REFRESH,
    async (): Promise<ShareUsageState> => {
      return refreshShareUsage();
    },
  );

  ipcMain.handle(
    IPC_CHANNELS.SHARE_USAGE_OPEN_UPGRADE,
    async (): Promise<void> => {
      await openUpgrade();
    },
  );
