...
const SNAP_API_BASE =
  process.env.CAPTUREFLOW_SNAP_API_BASE ?? "https://captureflow.xyz/api/s";
...
export async function uploadSnap(
  ...
  try {
    ...
    return {
      ok: false,
      error: "Failed to read captured snap",
      code: "read_failed",
    };
  }
...
export async function deleteSnap(
  ...
  try {
    const res = await fetch(
      `${SNAP_API_BASE}/snaps/${encodeURIComponent(id)}`,
      {
        method: "DELETE",
        headers: {
          authorization: `Bearer ${token}`,
