 ...
 * ShareApiClient
 * ──────────────
 * Pure HTTP plumbing for captureflow.xyz. Every call carries the
 * device-id header and (when present) the share-auth bearer. Lives in
 * main so the renderer's CSP stays narrow and the device-id never
...
// Always points at the deployed worker. Override via
// CAPTUREFLOW_SHARE_API_BASE for one-off staging tests.
...
// bound; the streamer buffers up to this threshold before POSTing a
// part. The hard cap per part on the worker is 100 MiB.
...
// Node's fetch body type doesn't accept a Uint8Array view directly;
// detach into a fresh ArrayBuffer (the simplest form that satisfies
// both Node and DOM lib typings).
...
// Poster upload uses the same bearer + device pair as multipart parts,
// but the worker expects `image/jpeg` (or png/webp) as the content type
// — `octet-stream` from postBytes would be rejected by the route's
// ALLOWED_TYPES gate. Keep this dedicated function so the caller never
// has to think about the content type.
