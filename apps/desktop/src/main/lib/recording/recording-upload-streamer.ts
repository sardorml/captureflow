...
} from "./recording-connectivity";
import { getActiveWorkspaceId } from "./recording-workspaces";
import {
  CHUNK_BYTES,
  postBytes,
  postJson,
  RecordingApiHttpError,
} from "./recording-api-client";
import { buildRecordingEditUrl } from "./recording-edit-url";
import {
  handleUploadError,
  type RecordingUploadFailure,
} from "./recording-error-handler";
import type {
  RecordingFinishMeta,
...
export async function startRecordingUpload(
  ...
  try {
    ...
    const unsubConnectivity = onRecordingConnectivityChange((state) => {
      if (!session) return;
    ...
    session = {
      ...
      paused: getRecordingConnectivity() === "offline",
      unsubConnectivity,
      aborted: false,
      finishing: false,
    };
    ...
    return {
      ok: true,
      slug: init.slug,
      editUrl: buildRecordingEditUrl(init.slug),
    };
  } catch (err) {
...
export function getActiveRecordingSlug(): string | null {
...
export async function finishRecordingUpload(
  ...
  if (s.aborted) {
    ...
    return {
      ok: false,
      error: "Recording session was aborted",
      code: "aborted",
    };
  }
  ...
  try {
    ...
    clearSession();
    return { ok: true, slug: s.slug, url: buildRecordingEditUrl(s.slug) };
  } catch (err) {
    const failure = handleUploadError(err, { slug: s.slug, phase: "finalize" });
    const partialUrl =
      s.screen.etags.length > 0 ? buildRecordingEditUrl(s.slug) : undefined;
    clearSession();
    return {
      ok: false,
...
export function abortRecordingUpload(): void {
  const s = session;
  if (!s) return;
  s.aborted = true;
  s.unsubConnectivity();
  logInfo("recording-streamer", `aborted: slug=${s.slug}`);
  session = null;
}
...
function failureToStart(f: RecordingUploadFailure): RecordingStartResult {
