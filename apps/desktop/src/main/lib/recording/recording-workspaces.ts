...
export function onWorkspacesChange(
  fn: (state: WorkspacesState) => void,
): () => void {
  events.on("change", fn);
  return () => {
    ...
    if (Array.isArray(parsed.workspaces)) {
      ...
      if (workspaces.length > 0) {
        const activeId =
          ...
          workspaces.some((w) => w.id === parsed.activeId)
            ? parsed.activeId
            : workspaces[0].id;
...
export async function refreshWorkspaces(): Promise<WorkspacesState> {
  ...
  const promise = (async (): Promise<WorkspacesState> => {
    ...
    try {
      ...
      if (res.status === 401) {
        // Leave state untouched; the /api/usage probe drives cleanup via recording-auth.
        return current;
      }
      if (!res.ok) {
      ...
      const activeId =
        prevActive && fresh.some((w) => w.id === prevActive)
          ? prevActive
          : fresh[0].id;
      ...
      if (changed) {
        logInfo(
          "workspaces",
          `refreshed: ${fresh.length} workspace(s), active=${activeId}`,
        );
        setState(next);
