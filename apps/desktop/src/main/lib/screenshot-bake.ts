...
// Must stay in sync with the web screenshot editor's GRADIENT_PRESETS.violet
...
// `cornerRadius={imgRenderW * 0.01}` in ScreenshotEditorImpl.tsx.
...
function decodeToRGBA(
  ...
  try {
    ...
    logWarn("screenshot-bake", `decode failed: ${String(err)}`);
    return null;
  }
}
...
export type ScreenshotBakeResult = {
...
export function bakeScreenshotWithDefaultBackground(
  input: Buffer,
): ScreenshotBakeResult {
  const t0 = Date.now();
  ...
  try {
    ...
    logWarn("screenshot-bake", `composed encode failed: ${String(err)}`);
    composedBytes = input;
  }
  ...
  try {
    ...
    logWarn("screenshot-bake", `source re-encode failed: ${String(err)}`);
    sourceBytes = input;
  }
