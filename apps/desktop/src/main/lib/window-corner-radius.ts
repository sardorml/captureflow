...
function getImpl(): ((view: Buffer, radius: number) => void) | null {
  ...
  try {
    ...
    const sel_registerName = objc.func(
      "void* sel_registerName(const char* name)",
    );
    const msgSend_layer = objc.func(
      "void* objc_msgSend(void* self, void* sel)",
    );
    const msgSend_setBool = objc.func(
      "void objc_msgSend(void* self, void* sel, bool arg)",
    );
    const msgSend_setDouble = objc.func(
      "void objc_msgSend(void* self, void* sel, double arg)",
    );
...
export function applyCustomCornerRadius(
  win: BrowserWindow,
  radius: number,
): void {
  const impl = getImpl();
