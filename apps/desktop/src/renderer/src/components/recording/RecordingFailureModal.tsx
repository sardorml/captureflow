...
import type { RecordingFailureState } from "../../../../shared/types";

export function RecordingFailureModal(): React.JSX.Element {
  const [state, setState] = useState<RecordingFailureState | null>(null);

  useEffect(() => {
    const off = window.electronAPI.onRecordingFailureInit((next) =>
      setState(next),
    );
    return off;
  }, []);
  ...
  const headline = state
    ? headlineFor(state.kind)
    : "Recording didn’t go through";
  const body = state?.message ?? "";
  const url = state?.kind === "partial" ? state.url : undefined;

  const onDone = (): void => window.electronAPI.recordingFailureClose();
  const onOpenLink = (): void => {
    if (url) window.electronAPI.recordingReadyOpenLink(url);
  };
...
function headlineFor(kind: RecordingFailureState["kind"]): string {
  switch (kind) {
    case "init-failed":
