...
const getHoverDeviceSnapshot = (): boolean => window.matchMedia(HOVER_QUERY).matches
...
const AnimatedTooltip = ({
  ...
  return (
    <span
      ...
      onMouseEnter={isHoverDevice && !disabled ? show : undefined}
      ...
      {createPortal(
        <AnimatePresence>
          {showTooltip && (
            <motion.span
              ref={tooltipRef}
              animate={shouldReduceMotion ? { opacity: 1 } : { opacity: 1, scale: 1, x: 0, y: 0 }}
              className={cn(
                'fixed z-50 w-max max-w-xs rounded-md bg-popover px-3 py-1.5 text-popover-foreground text-xs border border-border shadow-lg pointer-events-none',
                className
              )}
              style={{
                top: position?.top ?? 0,
