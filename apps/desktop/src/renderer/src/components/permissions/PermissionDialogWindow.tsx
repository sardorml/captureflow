...
const COPY: Record<
  ...
  Record<
    PermissionDialogInitPayload["variant"],
...
export function PermissionDialogWindow(): React.JSX.Element | null {
  const [payload, setPayload] = useState<PermissionDialogInitPayload | null>(
    null,
  );
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent): void => {
      if (e.key === "Escape")
        window.electronAPI.respondToPermissionDialog(false);
  ...
  return (
    ...
    >
      <img
        src={logoImg}
        alt=""
        className="permission-dialog__icon"
        draggable={false}
      />
