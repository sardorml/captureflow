...
import type { CaptureSource, RecordingMode, ShareAuthState } from '../../../shared/types'

export type RecordingStatus = 'idle' | 'preparing' | 'recording' | 'paused' | 'saving'
...
export const useRecordingStore = create<RecordingState>((set, get) => ({
  ...
  setVideoDevices: (devices): void => set({ videoDevices: devices, devicesReady: true }),
  ...
  setSelectedVideoDevice: (deviceId): void => {
    try {
      if (deviceId) {
        ...
        const label = get().videoDevices.find((d) => d.deviceId === deviceId)?.label
