...
export class CompositingShareEncoder {
  ...
  pushChunk(chunk: ShareChunk): void {
    if (this.finalized)
      throw new Error("CompositingShareEncoder.pushChunk after stop()");
    if (!this.ready || !this.decoder)
      throw new Error("CompositingShareEncoder not initialized");
  ...
  async stop(): Promise<ShareEncoderResult> {
    if (this.finalized)
      throw new Error("CompositingShareEncoder.stop called twice");
    this.finalized = true;
