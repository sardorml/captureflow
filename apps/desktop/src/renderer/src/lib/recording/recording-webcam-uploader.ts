...
// an MP4-in-.webm file freezes the recording player on the first frame.
...
export class RecordingWebcamUploader {
  ...
  start(inputs: RecordingWebcamUploaderInputs): void {
    ...
    if (videoTracks.length === 0) {
      throw new Error(
        "RecordingWebcamUploader: no video tracks on webcam stream",
      );
    }
    ...
    if (!mimeType) {
      throw new Error(
        "RecordingWebcamUploader: no supported mimeType for webcam+mic recorder",
      );
    }
    ...
    recorder.ondataavailable = (e) => {
      ...
      const promise = (async () => {
        ...
        window.electronAPI.recordingPartWebcam(buf);
      })().catch((err) => {
        logRendererWarn(
          `recording-webcam: chunk forward failed (${String(err)})`,
        );
      });
    ...
    logRendererInfo(`recording-webcam: started (mime=${mimeType})`);
  }
  ...
  async stop(): Promise<RecordingWebcamUploaderResult> {
    ...
    this.pendingChunks = [];
    this.cleanupStream();
    logRendererInfo(`recording-webcam: stopped (${this.totalBytes}B total)`);
