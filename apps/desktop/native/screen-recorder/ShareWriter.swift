// ShareWriter.swift
// ──────────────────
// Parallel H.264 encoder for the Instant Share pipeline. Sits next
...
//   1. Different target resolution (1280×720 share vs full capture
//      size for the local recording).
//   2. Different bitrate budget (4 Mbps share vs quality-based local
...
// (`StreamCapture.onShareFrame`) hands the same buffer to the share
...
final class ShareWriter {
  ...
  // 1280×720 (1.78:1) canvas. Without this the share has black bars
  ...
  private let writeQueue = DispatchQueue(label: "captureflow.share-writer.write")
  ...
  func start() throws {
    ...
    log(.info, "share-writer", "ready, deferring encoder until first frame")
  }
  ...
  private func setupEncoder(inWidth: Int, inHeight: Int) throws {
    ...
    guard status == noErr, let session = s else {
      throw NSError(
        domain: "ShareWriter", code: Int(status),
    ...
    // 2-second keyframe interval matches the renderer-side ShareEncoder
  ...
  /// Push one frame from the share-tap closure. The pixel buffer is
  ...
  func pushFrame(_ sampleBuffer: CMSampleBuffer) {
    ...
    // the canvas — paints as a 1-px black band on the encoded share.
  ...
  func pushAudio(_ sampleBuffer: CMSampleBuffer) {
    ...
    if firstAudioPts == nil {
      firstAudioPts = pts
      log(.info, "share-writer", "pushAudio: first frame, pts=\(pts.seconds)s")
    }
    ...
    guard
      ...
      log(.warn, "share-writer", "pushAudio: no format description / ASBD on sample buffer")
      return
    }
    var asbd = asbdPtr.pointee
    guard let inputFormat = AVAudioFormat(streamDescription: &asbd) else {
      log(.warn, "share-writer", "pushAudio: AVAudioFormat(streamDescription:) returned nil")
      return
    }
  ...
  private func setupAudioConverter(inputFormat: AVAudioFormat) -> Bool {
    var aacDesc = AudioStreamBasicDescription(
      mSampleRate: ShareWriter.aacOutputSampleRate,
      mFormatID: kAudioFormatMPEG4AAC,
      mFormatFlags: 0,
      mBytesPerPacket: 0,
      mFramesPerPacket: UInt32(ShareWriter.aacFramesPerPacket),
      mBytesPerFrame: 0,
      mChannelsPerFrame: ShareWriter.aacOutputChannels,
      mBitsPerChannel: 0,
      mReserved: 0
    )
    ...
    converter.bitRate = Int(ShareWriter.aacBitRate)
  ...
  private func emitAACPackets(_ buffer: AVAudioCompressedBuffer) {
    ...
    for i in 0..<count {
      ...
      let ptsUs = Int64(
        Double(audioPacketCursorSamples) * 1_000_000 / ShareWriter.aacOutputSampleRate
      )
      audioPacketCursorSamples += ShareWriter.aacFramesPerPacket
      let nextPtsUs = Int64(
        Double(audioPacketCursorSamples) * 1_000_000 / ShareWriter.aacOutputSampleRate
      )
  ...
  private func emitAudioFormat() {
    ...
    var packet = Data()
    packet.append(0x03)
    packet.appendU32(UInt32(ShareWriter.aacOutputSampleRate))
    packet.appendU32(ShareWriter.aacOutputChannels)
  ...
  private func emitFormatDescription(_ desc: CMFormatDescription) {
    ...
    guard let avcc = buildAvcCBox(from: desc) else {
      log(.warn, "share-writer", "could not build avcC box from format desc")
      return
    }
    var packet = Data()
    packet.append(0x01)
