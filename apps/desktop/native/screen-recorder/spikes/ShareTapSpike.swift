...
// PTS as they pass through the share-tap fan-out closure on the primary
// SCStreamOutput. Originally tried as a parallel SCStreamOutput; that
// approach broke the primary handler entirely (see docs/spikes/share-tap.md
...
//
// Off by default. Enable per-recording with CAPTUREFLOW_SHARE_TAP_SPIKE=1
// in the recorder process's environment.

import CoreMedia
import Foundation

final class ShareTapSpike {
  ...
  func logStats() {
    ...
    FileHandle.standardError.write(
      Data(
        "[share-tap-spike] frames=\(frameCount) elapsed=\(String(format: "%.2f", elapsed))s fps=\(String(format: "%.2f", fps)) drops=\(droppedCount)\n"
          .utf8
      )
    )
  }
  ...
  func finalReport() {
    FileHandle.standardError.write(Data("[share-tap-spike] FINAL —\n".utf8))
    logStats()
  }
}
